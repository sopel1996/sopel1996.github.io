jQuery(document).ready(function () {
	(function ($) {
		...
		$('.t-datepicker-carrental').tDatePicker({
			iconDate: '',
			titleCheckIn: 'תאריך התחלה',
			titleCheckOut: 'תאריך סיום',
			titleToday: 'היום',
		...
		$('.t-datepicker-flight').tDatePicker({
			...
			.on('eventClickDay', function () {
				...
				$('.t-datepicker-carrental').tDatePicker('setStartDate', $('.t-datepicker-flight').tDatePicker('getDateInputs')[0]);
				$('.t-datepicker-carrental').tDatePicker('setEndDate', $('.t-datepicker-flight').tDatePicker('getDateInputs')[1]);
			});
		...
		$('#calculate').on('click', function () {
			...
			if (checkAges()) {
				...
				var carRentalStart = $('.t-datepicker-carrental').tDatePicker('getDates')[0];
				var carRentalEnd = $('.t-datepicker-carrental').tDatePicker('getDates')[1];
				...
				daysCarRental = daysDiff(carRentalStart, carRentalEnd);

				var usa = $('select[name="usa"]').val();

				console.log('%cRegular Days: ' + days + ', Etgari Days: ' + daysEtgari + ', Ski: ' + daysSki + 'Car Rental: ' + daysCarRental + ', USA:' + usa, 'color: red; font-weight: bold;');
				...
				table += '<tr>';
				table += '<th>חברה</th>';
				table += '<th><a href="https://travel.harel-group.co.il/abroad-policy/?guid=bee1d408-c6a7-410e-b4ee-ac690224bdd3&an=11055" target="new"><img src="img/svg/harel.svg" alt="הראל ביטוח נסיעות" /></a></th>';
				table += '<th><a href="https://sites.migdal.co.il/NahalApp/#/nahal/newPolicy/firstStep?id=201" target="new"><img src="img/svg/migdal.svg" alt="מגדל ביטוח נסיעות" /></a></th>';
				table += '<th><a href="https://online.fnx.co.il/Travel/?fnx_ANum=L9sXObxg984%3d&referrer=insurance-online.co.il#" target="new"><img src="img/svg/fenix.svg" alt="הפניקס ביטוח נסיעות" /></a></th>';
				table += '<th><a href="https://purchase.passportcard.co.il/existing?AffiliateId=a9SzUY8MyfY3QxofVY21vQ%3D%3D" target="new"><img src="img/svg/passportcard.svg" alt="פספורטכארד ביטוח נסיעות" /></a></th>';
				table += '<th><a href="https://apps.menoramivt.co.il/TopTravel/?agentNum=910140#miniSite" target="new"><img src="img/svg/menora.svg" alt="מנורה ביטוח נסיעות" /></a></th>';
				table += '<th><a href="https://www.clalbit.co.il/travelingisurance/claltravel?txtItemID=4457f06c-bd5c-42a1-b660-0c36e334beaa" target="new"><img src="img/svg/clal.svg" alt="כלל ביטוח נסיעות" /></a></th>';
				table += '</tr>';
				$('.fields li').each(function () {
					...
					$.each(data, function (company) {
						if (usa == '1' && company == 'harel') {
							return true;
						}

						if (usa == '0' && company == 'harelusa') {
							return true;
						}

						console.log('%c-- Company: ' + company, 'font-weight: bold;');

						price = checkFieldsGetPrice(personNum, $this, company, personAge, days, daysEtgari, daysSki, daysCarRental);
				...
				table += '<tr>';
				table += '<td>סה״כ</td>';
				$.each(total, function (company, price) {
					if (usa == '1' && company == 'harel') {
						return true;
					}
		...
		var checkDaysLimit = function (company, age, days) {
			var limit = 0;

			if (typeof data[company]['limit'] !== 'undefined') {
				$.each(data[company]['limit'], function (k, v) {
					if (eval(age + k)) {
						limit = v;
						return false;
					}
				...
				if (days <= limit) {
					console.log('%c---- Days of flight (' + days + ') are within the limit (' + limit + ')', 'color: orange');
					return true;
				} else {
					console.log('%c---- Days of flight (' + days + ') are more then limit (' + limit + ')', 'color: orange');
					return false;
				}
			} else {
				console.log('%c---- No Days Limit found in "' + company + '"', 'color: orange');
				return true;
			}
		}
		...
		// ------------------------------------
		// Calculate Accidents Price
		// ------------------------------------

		var calculateAccidents = function (company, age, days) {
			var price = 0;

			if (typeof data[company]['accidents'] !== 'undefined') {
				$.each(data[company]['accidents'], function (k, v) {
					if (eval(age + k)) {
						price = v;
						return false;
					}
				...
				if (price > 0 && company == 'harel' || company == 'clal' || company == 'fenix' || company == 'passportcard') {
					console.log('%c---- Accidents Price In "' + company + '" For ' + days + ' Days: ' + price * days + '$ (Price per day: ' + price + ')', 'color: gray');
					return price * days;
				} else {
					console.log('%c---- No Accidents Price Found Returned FALSE', 'color: gray');
					return false;
				}
			} else {
				console.log('%c---- No Accidents Price found in "' + company + '"', 'color: gray');
				return 0;
			}
		}
		// ------------------------------------
		...
		// ------------------------------------
		// Calculate Corona Price
		// ------------------------------------

		var calculateCorona = function (company, days, tavyarok, personAge) {
			if (typeof data[company]['corona'] !== 'undefined') {
				var price = data[company]['corona'];

				var finalprice = price * days;

				if (company == 'passportcard' && price * days > 50) {
					finalprice = 50;
				}

				if ((company == 'fenix' || company == 'menora') && tavyarok == "0") {
					price = price + 1;
					finalprice = price * days;
				}

				if (company == 'migdal' && personAge >= 61) {
					price = price + 7;
					finalprice = price * days;
				}

				console.log('%c---- Tav Yarok: ' + tavyarok, 'color: gray');
				console.log('%c---- Corona Price In "' + company + '" For ' + days + ' Days: ' + finalprice + '$ (Price per day: ' + price + ')', 'color: gray');
				return finalprice;
			} else {
				console.log('%c---- No Corona Price found in "' + company + '"', 'color: gray');
				return 0;
			}
		}
		...
		var calculateRescue = function (company, age, days) {
			var price = 0;
			...
			if (typeof data[company]['rescue'] !== 'undefined') {
				...
				*/
				...
				console.log('%c---- Rescue Price In "' + company + '" For ' + days + ' Days: ' + price * days + '$ (Price per day: ' + price + ')', 'color: gray');
				return price * days;
			} else {
		...
		var calculateCarRental = function (company, age, days) {
			var price = 0;

			if (typeof data[company]['carrental'] !== 'undefined') {
				$.each(data[company]['carrental'], function (k, v) {
					if (eval(age + k)) {
						price = v;
						return false;
					}
				...
				if (price > 0) {
					console.log('%c---- Car Rental Price In "' + company + '" For ' + days + ' Days: ' + price * days + '$ (Price per day: ' + price + ')', 'color: gray');
					//if(days > 2) {
					//	return price * (days-2);
					//} else {
					return price * days;
				...
				console.log('%c---- No Car Rental Price found in "' + company + '"', 'color: gray');
				return 0;
			}
		}

		// ------------------------------------
		// Calculate USA Addon Price
		// ------------------------------------

		var calculateUSA = function (company, age, days) {
			var price = 0;

			if (typeof data[company]['usa'] !== 'undefined') {
				$.each(data[company]['usa'], function (k, v) {
					if (eval(age + k)) {
						price = v;
						return false;
					}
				...
				if (price > 0) {
					console.log('%c---- USA Addon Price In "' + company + '" For ' + days + ' Days: ' + price * days + '$ (Price per day: ' + price + ')', 'color: gray');
					return price * days;
				}
			} else {
				console.log('%c---- No USA Addon Price found in "' + company + '"', 'color: gray');
				return 0;
			}
		}
		...
		// ------------------------------------
		// Check if USA selected
		// ------------------------------------
		/*
		var checkUSA = function() {
			var usa = $('select[name="usa"]').val();
	
			if (usa == "") {
				$('select[name="usa"]').parent().append('<div class="error">זהו שדה חובה</div>');
				return false;
			} else {
				return true;
			}
		...
		// Check all fields
		// ------------------------------------
		var checkFieldsGetPrice = function (personNum, $this, company, personAge, days, daysEtgari, daysSki, daysCarRental) {
			var priceTemp = 0;
			...
			var tavyarok = $this.find('select[name="tavyarok"]').val();
			var usa = $this.find('select[name="usa"]').val();
			var etgari = $('input[name="t-datepicker-etgari"]').prop('checked');
			var ski = $('input[name="t-datepicker-ski"]').prop('checked');
			var carrental = $('input[name="t-datepicker-carrental"]').prop('checked');

			console.log(carrental);

			if (checkDaysLimit(company, personAge, days) == false) {
				return 0
			} else {
				var price = calculateBasic(company, personAge, days);

				if (company == 'menora' && usa == "1") {
					price = 0;
				}
				...
				/*
				if(company == 'harel' && personAge > 85 && days > 15) {
							price = 0;}
				if(company == 'harel' && personAge > 80 && days > 20) {
							price = 0;}
				if(company == 'harel' && personAge > 75 && days > 20) {
							price = 0;}
				if(company == 'harel' && personAge > 70 && days > 60) {
							price = 0;}
				if(company == 'harel' && personAge > 60 && days > 60) {
							price = 0;}
				if(company == 'harel' && personAge >= 60 && days > 90) {
							price = 0;}
				...
				if (usa == 1) {
					price += calculateUSA(company, personAge, days);
				}
				...
				if (accidents == 1) {
					priceTemp = calculateAccidents(company, personAge, days);
				...
				if (rescue == 1) {
					price += calculateRescue(company, personAge, days);
				}
				...
				if (flightcancle == 2) {
					price += calculateFlightCancle(company, personAge, days);
					price += calculateCorona(company, days, tavyarok, personAge);
				}
				...
				if (carrental == true && personNum == 1) {
					priceTemp = calculateCarRental(company, personAge, daysCarRental);
